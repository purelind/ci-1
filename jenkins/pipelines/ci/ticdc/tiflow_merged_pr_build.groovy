            ...
            builds["build ticdc"] = {
                run_with_pod() {
                    container("golang") {
                        ...
                        deleteDir()
                        unstash 'tiflow'
                        ticdc_binary = "test/cdc/ci/merge_pr_build/${ghprbActualCommit}/ticdc-linux-amd64.tar.gz"
                        timeout(unit: 'MINUTES', time: 20) { 
                            dir("go/src/github.com/pingcap/tiflow") {
                                ...
                                curl -F ${ticdc_binary}=@${TARGET}.tar.gz ${FILE_SERVER_URL}/upload
                                """
                            }
                        }
                        ...
                        def paramsDocker = [
                                ...
                                string(name: "INPUT_BINARYS", value: ticdc_binary),
                                string(name: "REPO", value: "tiflow"),
                                string(name: "PRODUCT", value: "ticdc"),
                                string(name: "RELEASE_TAG", value: "v6.2.0-aplha"),
                                string(name: "DOCKERFILE", value: ticdc_dockerfile),
                                string(name: "RELEASE_DOCKER_IMAGES", value: ticdc_image),
                        ]
    ...
    stage("update commit status") {
        node("master") {
            ...
            def default_params = [
                    ...
                    string(name: 'CONTEXT', value: 'idc-jenkins-ci/build'),
